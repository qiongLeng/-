...
#define ESP8266_WIFI_INFO		"AT+CWJAP=\"nine\",\"11111111\"\r\n"//wifi的账号和密码
...
{
    unsigned char timeOut = 40;  // 40 × 500ms = 20秒
    ...
    while (timeOut--)
    {
        ...
        {
            UsartPrintf(&huart1, "  [OK] Got: %s\r\n", res);
            ESP8266_Clear();
            return 0;
        }
    }

    UsartPrintf(&huart1, "  [FAIL] Timeout, buf=%s\r\n", esp8266_buf);
    return 1;
}
...
void USART2_IRQHandler(void)
{
    ...
    {
        uint8_t dummy = (uint8_t)(huart2.Instance->DR & 0xFF);
